...
|---|---|---|---|---|
| `creator-strategy-router` | 四类人群 | 角色/目标/权限→采购或定位路径→交接包 | 主体与目标、账号能力、已有材料 | 路径结论、数据边界、下一步启动包 |
| `platform-positioning-benchmark` | 创作者/AI IP/产品GTM | 平台机制证据→内容参照系→定位→小实验 | 定位需求、账号能力、自有证据 | 证据账本、定位选项、内容实验 |
| `research-to-brief` | 四类人群 | 多源资料→声明台账→冲突→Brief→独立审查 | 研究问题、来源/权利清单 | 声明台账、Brief、审查记录 |
| `creator-portfolio` | 有授权采购入口的营销/PR/GTM | 入口确认→合规候选快照→质量→匹配→校准→预算组合 | 路由结论、采购权限、预算、已有数据/快照、数据许可 | 采集审计、特征表、评分契约、组合方案 |
